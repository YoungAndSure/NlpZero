...
class CbowModel(nn.Module):
  def __init__(self, vocab_size, hidden_size):
    super().__init__()
    self.l0 = nn.Embedding(vocab_size, hidden_size)
    self.l1 = nn.Embedding(vocab_size, hidden_size)
    self.activator = nn.Sigmoid()

  def forward(self, contexts, t):
    y = None
    for context in contexts :
      y = y + self.l0(context) if y is not None else self.l0(context)
    y = y * self.l1(t)
    y = self.activator(y)
    return y
...
for epoch in range(max_epoch) :
  for i in range(len(ori_targets)) :
    x = torch.from_numpy(np.array(ori_contexts[i])[:, np.newaxis])
    positive = torch.from_numpy(np.array([ori_targets[i]]))
    y = model.forward(x, positive)
    loss = loss_fn(y, torch.ones(1, dtype=torch.long))
    ...
    for negative in negatives :
      ...
      loss += loss_fn(y, torch.zeros(1, dtype=torch.long))
