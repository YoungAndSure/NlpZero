...
from dataset.easy_data import HelloW2vGenerateDataset
...
train_dataset = HelloW2vGenerateDataset()
test_dataset = HelloW2vGenerateDataset()
...
with torch.no_grad() :
  text = train_dataset.text[:train_dataset.window]
  print(text)
  generate_len = len(train_dataset.corpus) - train_dataset.window
  for i in range(generate_len) :
    context = text[-2:]
    ids = torch.tensor(train_dataset.to_ids(context))[torch.newaxis].to(device)
    center_id = model.predict(ids)
    for j in range(center_id.shape[0]) :
      center_word = train_dataset.to_word(center_id[j][0].item())
    text.append(center_word)
    print(context, center_word, text)